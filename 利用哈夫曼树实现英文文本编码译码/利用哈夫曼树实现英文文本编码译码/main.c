...
void Select(HuffmanNode *ht,int n,int *s1,int *s2){
    ...
    *s1=i;
    ...
    for (i+=1; i<n;i++) {
        if (!ht[i].parent) {
            if ((ht[i].weight.value<ht[*s1].weight.value)||(ht[i].weight.value==ht[*s1].weight.value)) {
                *s2=*s1;
                *s1=i;
            }
        }
    ...
    if (*s1==flag) {
        ...
        *s2=i;
        for (i+=1; i<n; i++) {
            if (ht[i].weight.value<ht[*s2].weight.value) {
                *s2=i;
            }
        }
    }
...
int main(int argc, const char * argv[]) {
    ...
    printf("*****%c %d %d %d %d*****",ht[0].weight.Alphabet,ht[0].weight.value,ht[0].parent,ht[0].LChild,ht[0].RChild);
    printf("\n");
