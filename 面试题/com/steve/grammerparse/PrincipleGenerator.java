...
public class PrincipleGenerator {
    ...
    public PrincipleGenerator() throws Exception{
        String filePath="./src/com/steve/grammerparse/PrincipleGeneratorSource";
    ...
    public ArrayList<String> getFollow(String s, String element, String track1) {
        ...
        if (Character.isUpperCase(s.charAt(0))) {
            ...
            for (int i = 0; i < in.size(); i++) {
                ...
                else if (one[1].indexOf(s) >= 0 && (olen - slen == one[1].indexOf(s) || slen == 2 ||
                        one[1].charAt(one[1].indexOf(s) + 1) != '’' && one[1].charAt(one[1].indexOf(s) + 1) != '\'')) {//如果在右产生式中真正存在需要求反推的字符,后面的条件控制它是真正存在，因为里面包含这个字符也不一定是真，就像E’中包含E，但这不是真正的包含
                    int index = -1;
                    index = one[1].indexOf(s, 0);
                    while (index >= 0) {if (olen - slen == index) {
                        result1 = getFollow(one[0], element, track1 + (char) ('a' + i));
                            result = addArrayString(result, result1);
                            result1.clear();
                        } else {int t = index + slen;
    ...
    public ArrayList<String> returnFirstofFollow(String s, String element, String track1, String one0, String one1, int index, int t) {
        ArrayList<String> result = new ArrayList<String>();
        ArrayList<String> result1 = new ArrayList<String>();
        ArrayList<String> beckFirst;
        String lsh;
        if (t + 1 < one1.length() && (one1.charAt(t + 1) == '’' || one1.charAt(t + 1) == '\''))
